...
sampleSolution = [0, 1, 20, 9, 3, 12, 26, 25, 24, 4, 23, 22, 21, 0, 13, 2, 15, 14, 6, 27, 5, 16, 17, 8, 18, 0, 7, 19, 11, 10, 0]
geneNum = 100  # 种群数量
...
# 需求量
t = [0, 0.8, 1.2, 0.4, 0.8, 0.4, 0.8, 0.8, 0.4, 0.8, 0.8, 15, 0.4, 0.4, 0.8, 0.4, 0.4, 1.5, 1.2, 0.4, 1.2, 0.4, 1.2,
     0.12, 0.64, 0.64, 0.0, 0.0]
t = [i*10 for i in t]
# 最早到达时间
...
# return a bunch of random population
def getRandomGenes(size):
    population = []
    for i in range(size):
        population.append(Gene("Gene "+str(i)))
    return population


# 计算适应度和
def getSumFit(population):
    sumFit = 0
...
# 更新选择概率
def updateChooseProb(population):
    sumFit = getSumFit(population)
    for gene in population:
        gene.updateChooseProb(sumFit)


# 计算累计概率
def getSumProb(population):
    sum = 0
    for gene in population:
        sum += gene.chooseProb
    return sum
...
def choose(population):
    ...
    # sort population with respect to chooseProb
    key = lambda gene: gene.chooseProb
    population.sort(reverse=True, key=key)
...
def cross(population):
    ...
    for i in range(0, len(population), 2):
        crossPair(population[i], population[i+1], crossedGenes)
    return crossedGenes



# 合并
def mergeGenes(population, crossedGenes):
    # sort population with respect to chooseProb
    key = lambda gene: gene.chooseProb
    population.sort(reverse=True, key=key)
    pos = geneNum - 1
...
if __name__ == "__main__" and not DEBUG:
    ...
    for i in tqdm(range(generationNum)):
        updateChooseProb(population)
        sumProb = getSumProb(population)
        chosenGenes = choose(deepcopy(population))   # 选择
        crossedGenes = cross(chosenGenes)   # 交叉
        population = mergeGenes(population, crossedGenes) # 复制交叉至子代种群
        population = vary(population) # under construction
    # sort population with respect to chooseProb
    key = lambda gene: gene.fit
    population.sort(reverse=True, key=key)   # 以fit对种群排序
    print('\r\n')
    print('data:', population[0].data)
    print('fit:', population[0].fit)
    population[0].plot() # 画出来
