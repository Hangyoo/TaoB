...
def get_options(args=None):
    ...
    parser.add_argument('--update_step_test', type=int, default=100, help='Finetuning steps') #10 测试情况下微调的次数

    # train
    parser.add_argument('--is_train', action='store_true', default=False, help='whether to train')
    parser.add_argument('--is_test', action='store_true', default=True, help='whether to test')
    ...
    # cuda (在这里可指定运行的GPU)
    parser.add_argument('--CUDA_VISIBLE_ID', default="0") # 改成2可以使用并行GPU运算
    parser.add_argument('--no_cuda', action='store_true', default = False, help='whether to disable cuda')

    # load
    parser.add_argument('--is_load', action='store_true', default=True, help='whether to load model') # 加载已经测试好的模型进行微调
    ...
    # parser.add_argument('--load_name', default='meta-model-1500.pt', help='Name to load model parameters and optimizer state from')
    # parser.add_argument('--load_path', default='meta-model-1500.pt', help='Name to load model parameters and optimizer state from')
    # parser.add_argument('--load_path', default='meta-model-MOTSP50.pt', help = 'Path to load model parameters and optimizer state from') # 准确的
