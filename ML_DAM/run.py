...
problem = load_problem(opts.problem) # tsp
...
class Meta(nn.Module):
    ...
    def finetunning(self, weight=None):
        ...
        print("--------------------------------------------")
        print("当前对应的weight:{}, {}".format(weight[0].item(), weight[1].item()))
        ...
        for update_id, batch in enumerate(finetunning_dataloader):  # for update_id in range(self.update_step_test):
            ...
            cost, log_likelihood = actor_model(x) # cost:{list:2 [[目标函数1],[目标函数2]]} log_likehood:512
            bl_val, bl_loss = baseline.eval(x, cost, weight) # bl_val:512 bl_loss:数
            c = weight[0] * cost[0] + weight[1] * cost[1] # c:512
            # todo 可以修改MOPO的地方
            reinforce_loss = ((c - bl_val) * log_likelihood).mean()
            loss = reinforce_loss + bl_loss
            # 计算在不同权重下加权值,验证微调的有效性
            converge.append((weight[0]*cost[0].mean() + weight[1]*cost[1].mean()).item())
        ...
        return actor_model, baseline, converge

    def validate(self, dataset, actor_model=None):
        '''给一个actor_model, 验证其在dataset上的表现,返回两个目标的均值
           无 actor_model 情况下: 验证子模型
        '''
        if actor_model is None:
            # 若 actor_model 不为 None, 则训练 meta_model
        ...
        lcost_obj1 = []
        lcost_obj2 = []
        for batch in DataLoader(dataset, batch_size=opts.eval_batch_size): # eval_batch_size = 512
            x = move_to(batch, opts.device)
            cost, log_likelihood = actor_model(x)
            lcost_obj1.append(cost[0]) # [(512,),(512,),...]
            lcost_obj2.append(cost[1]) # [(512,),(512,),...]
        cost_obj = [torch.cat(lcost_obj1, 0), torch.cat(lcost_obj2, 0)] # [lcost_obj1, lcost_obj2]

        torch.cuda.empty_cache() # 释放torch缓存
        cost_obj0_mean = cost_obj[0].mean()
        cost_obj1_mean = cost_obj[1].mean()
        return [cost_obj0_mean, cost_obj1_mean] # 返回cost1的均值, cost2的均值 [9.8343,7.2466]
...
}.get(opts.model, None) # attention(默认), pointer
assert model_class is not None, "Unknown model: {}".format(model_class)

# 当前的model_class为attention
model = model_class(
    opts.embedding_dim,
    opts.hidden_dim, # 128
    problem, # tsp
    n_encode_layers=opts.n_encode_layers, # 3
    mask_inner=True,
    mask_logits=True,
    normalization=opts.normalization, # BN
...
# 执行训练时的代码
if opts.is_train:
    print(f'模型的保存位置为:{opts.save_dir}')
    if opts.meta_algorithm == "maml":
        ...
        for epoch_id in range(opts.start_epoch, opts.start_epoch + opts.training_epochs):
            ...
            for batch_id, query_batch in enumerate(tqdm(query_dataloader)):
                ...
                if batch_id % 30 == 0:
                    weight = torch.tensor([0.0, 1.0])
                    actor_model, baseline, converge = maml_learner.finetunning(weight)
        ...
        print('Meta采用reptile方式运行')
        reptile_learner = meta_learner
        for meta_iteration_id in tqdm(range(opts.start_meta_iteration, opts.meta_iterations)): # (0,10000)
            meta_lr = opts.meta_lr * (1. - meta_iteration_id / float(opts.meta_iterations))
            if meta_iteration_id % 30 == 0:
                # 每30代对模型进行一次微调,并计算模型在验证集上的表现
                print("meta_iteration_id:{}/{}, meta_learning_rate:{}, update_lr:{}, finetunning_lr:{}".format(
                    meta_iteration_id,
                    opts.meta_iterations,
                    reptile_learner.meta_lr,
                    reptile_learner.sub_train_lr,
                    reptile_learner.finetunning_lr))
                weight = torch.tensor([0.0, 1.0])
                actor_model, baseline, converge = reptile_learner.finetunning(weight) # 根据权重进行微调
                cost_obj = reptile_learner.validate(val_dataset, actor_model)
                print("validate_cost_obj1:{}, validate_cost_obj2:{}".format(cost_obj[0], cost_obj[1])) # 在验证集上的目标值

            # t1 = time.time()
            reptile_learner(meta_lr=meta_lr) # 1650上大致130s
            # t2 = time.time()
...
if opts.is_test and opts.is_transfer is not True:
    # 生成验证集数据
    val_dataset = problem.make_dataset(  # opts.val_size = 10000
        size=opts.graph_size, num_samples=1, filename=opts.val_dataset, distribution=opts.data_distribution)

    # 采用Benchmark数据集
    num_nodes = 200
    path = r'C:\Users\Hangyu\PycharmProjects\TaoB\ML_TSP_4static\Post_process\krodata'
    x1 = np.loadtxt(path + '/kroA%d.tsp' % num_nodes, skiprows=6, usecols=(1, 2), delimiter=' ',
                    dtype=float)  # usecols=(1, 2) 指定读取1,2列
    x1 = x1 / (np.max(x1, 0))  # 数据归一化
    x2 = np.loadtxt(path + '/kroB%d.tsp' % num_nodes, skiprows=6, usecols=(1, 2), delimiter=' ',
                    dtype=float)  # usecols=(1, 2) 指定读取1,2列
    x2 = x2 / (np.max(x2, 0))  # 数据归一化
    x = np.concatenate((x1, x2), axis=1)  # (100,2) --> (100,4) 改变了第1维度, 即axis=1
    x = x.reshape(num_nodes, 4)
    ...
    # 返回cost1的均值, cost2的均值 [9.8343,7.2466]
    ...
    for i in range(1):
        if i < opts.resume_i:
            continue
        weight = weights[i]
        actor_model, baseline, converge = meta_learner.finetunning(weight)   # 对当前权重进行微调
        cost_obj = meta_learner.validate(val_dataset, actor_model) # 验证微调后的模型
        print("cost_obj1:{}, cost_obj2:{}".format(cost_obj[0].item(), cost_obj[1].item()))
    ...
    print('子模型在不同权重下的结果为:',result)
    with open(f'KroAB{num_nodes}.pkl', 'wb') as f:
