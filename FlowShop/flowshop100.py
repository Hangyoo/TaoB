...
parser.add_argument('--file', default='dataset.csv', type=str)          # 读取数据集，忽略
parser.add_argument('--instance_no', default=1, type=int)               # 忽略
parser.add_argument('--instance_all', default=False, type=bool)         #忽略
parser.add_argument('--population', default=20, type=int)               # 种群大小
parser.add_argument('--crossover_rate', default=0.8, type=float)        # 交叉概率
parser.add_argument('--mutation_rate', default=0.2, type=float)         # 变异概率
parser.add_argument('--mutation_selection', default=0.5, type=float)    # 忽略
parser.add_argument('--iteration', default=100, type=int)               # 迭代次数
parser.add_argument('--mutation_operator', default='base', type=str)    #变异方式(两点变异)


def read_file(file_name, instance):
    data_tmp = pd.read_csv('dataset.csv')
    data1 = data_tmp.values.tolist()
    num_m = 59
    num_job = 100
    global data_new
...
def crossover(population_list, population, crossover_rate, num_job):
    ...
    for m in range(int(population/2)):
        ...
        if crossover_rate >= crossover_prob:
            ...
            # 两点交叉，以fix_num为Cutting Point,为避免非法重复，子代在Cutting Point之后的顺序由另一父辈对应顺序决定。
