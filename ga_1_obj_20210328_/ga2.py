...
def evolve(pop, xMin, xMax, yMin, yMax, f_max,generation,maxGen,retain=0.05, select=0.05, crossover=0.6, mutate=0.01):
	"""
	种群进化
    pop:                Population.
    xMin/Max, yMin/Max: 变量边界.
    retain:             精英个体保留比例.
    select:             随机保留父代5%的个体进入子代(为了保证种群多样性，最小为2%).
    mutate:             变异概率.
    """
	...
	for indi in graded:
		if mutate > random.random():
			...
			G = generation / maxGen
			# 期望
			E = np.var([fitness(x) for x in pop])
			M = (f_avg + 1) / math.sqrt(E)
			if f_max >= fitness(indi) >= f_avg:
				mutate = 0.03 - (0.05-0.01)/(1+math.exp(G/M))
			elif fitness(indi) < f_avg:
				mutate = 0.02
	...
	while len(children) < desiredLength:
		...
		# E = np.var([fitness(x) for x in pop])
		# M = (f_avg + 1) / math.sqrt(E)
		# if fitness(indi) == f_max:
		# 	crossover = 0.1
		# elif f_max >= fitness(indi) >= f_avg:
		# 	crossover = 0.6 - (0.9 - 0.6) / (1 + math.exp(-G / M))
...
while generation < maxGen:
	generation += 1
	pop = evolve(pop, xMin, xMax, yMin, yMax,genFitness,generation,maxGen)
...
# 保存结果
with open('data_ga2.pkl','wb') as f:
