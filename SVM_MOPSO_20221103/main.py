...
import numpy as np
import pandas as pd
from geatpy.algorithms.soeas.GA.EGA.soea_EGA_templet import soea_EGA_templet as soea_EPSO_templet
from SVM_MOPSO_20221103.problem import MyProblem
...
if __name__ == '__main__':
    ...
    myAlgorithm = soea_EPSO_templet(problem, population)  # 实例化一个算法模板对象
    ...
    df2.to_csv('Objective.csv',header=None, index=None)
     ...
     # # 绘图
     # plt.scatter([x[0] for x in NDSet.ObjV],[x[1] for x in NDSet.ObjV])
     # plt.xlabel('loss')
     # plt.ylabel('power')
     # plt.show()
