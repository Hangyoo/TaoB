...
def translate_decoded_to_gantt(machine_operations):
    ...
    for idx, machine in enumerate(machine_operations):
        if int(idx + 1) < 5:
            ...
            operations = []
            machine_name = "Machine-{}".format(5)
            for operation in machine:
                operations.append([operation[3], operation[3] + operation[1]/2, operation[0]])
            data[machine_name] = operations

            operations = []
            machine_name = "Machine-{}".format(6)
            for operation in machine:
                Str,ope = operation[0].split("-")
                Str += ("-" + str(int(ope) + 1))
                operations.append([operation[3] + operation[1] / 2, operation[3] + operation[1], Str])
            data[machine_name] = operations
        else:
            operations = []
            machine_name = "Machine-{}".format(idx + 2)
            for operation in machine:
                Str, ope = operation[0].split("-")
                Str = Str + "-" + str(int(ope) + 1)
                operations.append([operation[3], operation[3] + operation[1], Str])
            data[machine_name] = operations
    data.popitem()
    return data
