# -*- coding: utf-8 -*-
"""
Created on Sat Jan  2 14:00:00 2021
    ...
    def aimFunc(self, pop):  # 目标函数
        ...
        for i in range(popsize):
            for j in range(len(Vars[0, :])):
                var = int(Vars[i, [j]])
                each_time[i][var] += time[j]
        # b = Vars.tolist()
        # index = [0]*9
        # for i in range(popsize):
        #     for j in range(9):
        #         index[j] = [x for (x,m) in enumerate(b[0]) if m==j]
        #         for k in index[j]:
        #             each_time[i][j] += time[k]
        ...
        pop.cv = np.hstack([336-f])'''
        
        '''f = []
        for i in range(popsize):
            f.append([max(each_time[i])-min(each_time[i])])
        f = np.array(f)
        pop.ObjV = f  # 把求得的目标函数值赋值给种群pop的ObjV
        #pop.cv = np.hstack([336-f])'''
...
myAlgorithm.MAXGEN = 8000 # 最大进化代数
