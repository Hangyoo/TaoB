...
def adapt(x):
    ...
    for args in x:
        ...
        val = 100 / (abs(args[0]+1) + abs(args[1]) + abs(args[2]-1) + abs(args[3]+2) + abs(args[4]+3) + abs(args[5]-2)+abs(args[6]-3)+1)
        f.append(val)
    return f
    ...
    for i in range(len(f1)):
        if i % 2 != 0:
            ...
            for k in range(rand + 1, len(f2[0])):
                ...
                # 解的修复
                child1 = min(10, max(child1,-10))
                child2 = min(10, max(child2,-10))
...
def variation_ml(f):
    ...
    for n, i in enumerate(c):
        f[n][0] = -1 if True else 0
        f[n][3] = -2 if True else 0
    for n, i in enumerate(c):
        if (-1 in i):
            for m, j in enumerate(i):
                if j == -1:
                    # print('变异的位置：', n, m)
                    f[n][m] = np.random.rand() * 8 - 4  # 随机数替代变异数
        # 对第二类标签的个体进行变异
...
def variation_nl(f):
    ...
    c = np.where(c < 0.1, -1, c)  #判断随机数小于0.1为变异
    for n, i in enumerate(c):
        f[n][1] = 0 if True else 0
        if (-1 in i):
            for m, j in enumerate(i):
                if j == -1:
                    f[n][6] = 3 if True else 0
...
def mutation_polynomial(f):
    ...
    for n, i in enumerate(c):
        if (-1 in i):
            for m, j in enumerate(i):
                f[n][2] = 1.0 if True else 0
                if j == -1:
                    # 多项式变异
                    y = f[n][m]
                    delta1 = 1.0*(y+10)/20
                    delta2 = 1.0*(10-y)/20
                    eta_m = 0.2 # 多项式变异的分布指数，根据经验值设定
                    mut_pow = 1.0 / (eta_m + 1)
                    r = random.random()
                    if r <= 0.5:
                        xy = 1 - delta1
...
if __name__ == '__main__':
    ...
    for i in range(gen):   #算法迭代一千次
        ...
        if best < best1:    #判断每次变异后的最有适应值与选择出来的适应值，哪个更优，选择最优的那个
            best = best1
            # 找到最优解并记录
            best_idiv = x[f.index(best1)]
        best_list.append(best)
        if i % 1000 == 0:
            print('第', i, '次循环：')
    print('循环100次的所有最大值：')
    print('最优解为：',best)
    print('变量取值为:',best_idiv)

    plt.plot(range(gen),best_list)
