...
#管径备选值
#主干管最小管径是400
D_list = [200,300,400.0, 450.0, 500.0, 600.0, 700.0, 800.0, 1000.0, 1200.0]
...
# print(sorted([(x.name, x.D, x.v, x.i, x.offset_out) for x in pipe_set.values()], key=lambda y: y[-1]))
# print(sorted([(x.name, x.invert, x.depth) for x in node_set.values()], key=lambda y: y[2]))
def change_pipe(m):
    change_number = pd.read_excel('待优化.xlsx').values
    pipe_List=[]
    for k in change_number:
        for j in k:
            pipe_List.append(j.strip())
    N= len(pipe_List)#待优化管径数量
    random.seed(N)
    slice=random.sample(pipe_List,m)#从待优化管段随机选择几个管段
    for pipe_name, pipe_object in pipe_set.items():
        #如果pipe属于选定扩增的管段
        if pipe_name in slice:
            for d in D_list:
                if pipe_object.D==d:
                    i=D_list.index(d)#管段管径在D_list中对应的序号
                    if i<len(D_list)-1:
                        pipe_object.D=D_list[i+1]#管径扩增一级
    #重新计算埋深参数
    for node_name, node_object in node_set.items():
        if node_object.root == None:
            #如果没有下游node，即outfall，设管底标高为-1.5m
...
##### create important parts of the inp file生产inp文件
