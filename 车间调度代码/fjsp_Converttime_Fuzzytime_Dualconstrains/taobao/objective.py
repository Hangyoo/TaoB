...
def TimeTaken(os_ms, parameters, convert):  # 个体（[os],[ms]）
    os, ms, ws = os_ms[0], os_ms[1], os_ms[2]
    decoded = decoding.decode(parameters, os, ms, ws, convert)
...
def FuzzyTimeTaken(os_ms, parameters, convert, num):  # 个体（[os],[ms]）
    os, ms, ws = os_ms[0], os_ms[1], os_ms[2]
    if num == 0 or num == 2:
        decoded = decoding.Fuzzydecode(parameters, os, ms, ws, convert, num)
    else:
        decoded = decoding.decode(parameters, os, ms, ws, convert)
    # 获取每台机器上最大完工时间
    max_per_machine = []
...
def maxload(os_ms, parameters, convert):
    decoded = decoding.decode(parameters, os_ms[0], os_ms[1], os_ms[2], convert)
